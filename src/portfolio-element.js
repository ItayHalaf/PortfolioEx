...
export default function PortfolioElement (props) {
    const {url,id,title,onDelete,description} = props;
    return (
        <div className="portfolio-element">
            <img onClick={() => props.onClick(id)} className="image" src={url} alt="" />
            <div>{title}</div>
            <div>{description}</div>
            <button onClick={() => onDelete(id)} className="button"><i className="fa fa-trash"></i></button>
            <button onClick={()=>props.changeToEdit(id)} className="button"><i className="fa fa-edit"></i></button>
        </div>
    );
}
